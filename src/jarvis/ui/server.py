...
class UiServer:
    ...
    async def _serve(self) -> None:
        ...
        async with websockets.serve(
            ...
            # After the port is known, so a config of 0 publishes what the OS
            # actually gave us rather than the zero.
    ...
    def write_hud_config(self, app_dir: Path | None = None) -> list[Path]:
        ...
        were all validated and none of them reached the HUD. main.js read
        ``window.JARVIS_PORT`` and main.rs read ``JARVIS_HUD_POSITION``, and
        style.css carried a comment saying the accent was "injected at build
        time" by an injector that did not exist anywhere in the tree. So
        changing the port moved the server and left the HUD dialling 8765
        forever, with no error beyond a reconnect that never succeeded.

        Written rather than templated at build time because the config is the
        ...
        Returns:
            ...
            which is the normal case for a source checkout that has never built
            the HUD.
        """
        ui = self._config.ui
        payload = {
            "JARVIS_HOST": ui.host,
            # The bound port, not the configured one: with port 0 the OS picks,
            # and the HUD needs the one that is actually listening.
            "JARVIS_PORT": self._port,
            "JARVIS_ACCENT": ui.accent_color,
            "JARVIS_HUD_POSITION": ui.hud_position,
        }
        body = "// Generated by jarvis.ui.server. Edits here are overwritten.\n" + "".join(
        ...
        for target in (root / "src", root / "dist"):
            if not target.is_dir():
                continue
            try:
                (target / "hud-config.js").write_text(body, encoding="utf-8")
            except OSError:
                _log.warning(
                    "could not write the HUD config",
                    extra={"context": {"path": str(target)}},
                )
                continue
            written.append(target / "hud-config.js")

        if written:
            _log.debug(
                "wrote the HUD config",
                extra={"context": {"port": self._port, "files": len(written)}},
            )
        return written
